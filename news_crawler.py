...
PATH = '/Users/ritalliou/Desktop/IIS/News Ideology/myCrawlerResult/politics/' if POLITICS else '/Users/ritalliou/Desktop/IIS/News Ideology/myCrawlerResult/sports/'
...
class my_news_crawler():
    ...
    def __init__(self):
        ...
        self.proxies = []
        self.proxy = self.get_random_proxies()
    ...
    def get_url_text(self, url):
        ...
        while 1:
            try:
                ...
                print('0.0 ERROR in loading '+'\t'+url); print(e)
                print("Connection refused by the server..\nLet me sleep for 5 seconds\nZZzzzz...")
                sleep(5)
                #print("Was a nice sleep, now let me continue...")

                session = requests.Session()   # load cookie data to imitate browser
                session.headers.update({'User-Agent':str(UserAgent().random)}) # change user agent in header
                random_prox = self.get_random_proxies()  # change proxy 
                print('Change proxy to '+random_prox)
                self.proxy = {"http": 'http://'+random_prox, "https://": 'https'+random_prox}          
                continue
    ...
    def chinatimes_all_reader(self):
        ...
        while n < 15:
            try:
                url = 'https://www.chinatimes.com/politic/total?page='+str(n)+'&chdtv' if POLITICS else 'https://www.chinatimes.com/sports/baseball/total?page='+str(n)+'&chdtv'
    ...
    def chinatimes_reader(self, url):
        ...
        if res.find_all('figure', attrs={'data-media-type':'video'}) == None: video = []
        else:
    ...
    def liberty_reader(self, url):
        ...
        if res.select('.whitecon .viewtime')!=[]:
            time_str = res.find('meta', attrs={'name':'pubdate'})['content']
            date_obj = datetime.strptime(time_str, '%Y-%m-%dT%H:%M:%SZ') # ex. 2019-09-20T05:30:00Z
        else:
        ...
        author = context.split(u'〔')[1].split(u'／')[0] if len(context.split(u'〔'))>1 else None
    ...
    def udn_all_reader_1(self):
        ...
        for drink in drinks: # .class # #id
            ...
            if news_url == self.last_news_url: # reach has read news
                ...
                news_dict = self.udn_reader_1(news_url)
                all_news.insert(0, news_dict)


    def udn_reader_1(self, url):
        text = self.get_url_text(url)
        res = BeautifulSoup(text, features='lxml')
    ...
    def udn_all_reader_2(self):
        ...
        for drink in drinks: # .class # #id
            ...
            if news_url == self.last_news_url: # reach has read news
                ...
                news_dict = self.udn_reader_2(news_url)
                all_news.insert(0, news_dict)
    ...
    def udn_reader_2(self, url):
        text = self.get_url_text(url)
        res = BeautifulSoup(text, features='lxml')

        title = res.select(".story_art_title")[0].contents[0].strip('\n')
        tag_list = res.find('meta', attrs={'name':'news_keywords'})['content'].split(u',')
        author = res.select('.story_bady_info_author')[0].text
    ...
    def sten_all_reader(self):      
        ...
        while n < 80:
            try:
                url = 'https://www.setn.com/ViewAll.aspx?PageGroupID=6&p='+str(n) if POLITICS else 'https://www.setn.com/ViewAll.aspx?PageGroupID=34&p='+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except:
                print("Reach the end page.")
                break
    ...
    def era_reader(self, url):
        ...
        author = res.select('.author')[0].text if res.select('.author')!=[] else None
        tag_list = res.find('meta', attrs={'name':'keywords'})['content'].split(u',')
    ...
    def ustv_all_reader(self):
        ...
        while n < 7:
            try:
                url = 'https://www.ustv.com.tw/UstvMedia/news/107?page='+str(n) if POLITICS else 'https://www.ustv.com.tw/UstvMedia/news/108?page='+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except:
                print("Reach the end page.")
                break
    ...
    def cna_all_reader(self):
        for n in xrange(1,6): # click num of button "more"
            try:
                url = 'https://www.cna.com.tw/cna2018api/api/simplelist/categorycode/aipl/pageidx/'+str(n) if POLITICS else 'https://www.cna.com.tw/cna2018api/api/simplelist/categorycode/aspt/pageidx/'+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except:
                print("Reach the end page.")
                break
    ...
    def thenewslens_all_reader(self):       
        ...
        while n > 0:
            try:
                url = 'https://www.thenewslens.com/category/politics?page='+str(n) if POLITICS else 'https://www.thenewslens.com/category/sports?page='+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except Exception as e:
                print(e)
                print("Reach the end page.")
    ...
    def epochtimes_all_reader(self):
        ...
        while n > 0:
            try:
                url = 'https://www.epochtimes.com/b5/ncid1077884_'+str(n)+'.htm' if POLITICS else 'https://www.epochtimes.com/b5/nsc1014_'+str(n)+'.htm'
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except Exception as e:
                print(e)
                print("Reach the end page.")
    ...
    def newtalk_all_reader(self):   
        ...
        while n > 0:
            try:
                url = 'https://newtalk.tw/news/subcategory/2/%E6%94%BF%E6%B2%BB/'+str(n) if POLITICS else 'https://newtalk.tw/news/subcategory/102/%E9%AB%94%E8%82%B2/'+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except Exception as e:
                print(e)
                print("Reach the end page.")
    ...
    def worldjournal_all_reader(self):
        ...
        while n < 16:
            try:
                url = 'https://www.worldjournal.com/topic/%e5%8f%b0%e7%81%a3%e6%96%b0%e8%81%9e%e7%b8%bd%e8%a6%bd/?pno='+str(n) if POLITICS else 'https://www.worldjournal.com/category/%e9%81%8b%e5%8b%95/?pno='+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except Exception as e:
                print(e)
                print("Reach the end page.")
    ...
    def mypeople_all_reader(self):
        url = 'http://www.mypeople.tw/index.php?r=site/index&id=22848&channel=1' if POLITICS else 'http://www.mypeople.tw/index.php?r=site/index&id=5447&channel=1'
        self.browser.get(url)
        jsCode = "var q=document.documentElement.scrollTop=100000" # control distance of scroller on the right
        for x in range(0,15): 
            self.browser.execute_script(jsCode) # keep scrolling to the bottom
    ...
    def taronews_all_reader(self):  
        ...
        while n < 173:
            try:
                url = 'https://taronews.tw/category/politics/page/'+str(n) if POLITICS else 'https://taronews.tw/category/sport/baseball/page/'+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except Exception as e:
                print(e)
                print("Reach the end page.")
    ...
    def Pchomenews_all_reader(self):
        ...
        while n < 20:
            try:
                url = 'http://news.pchome.com.tw/cat/politics/'+str(n) if POLITICS else 'http://news.pchome.com.tw/cat/sport/'+str(n)
                #resp = urllib2.urlopen(url).getcode() # check url exist
            except:
                print("Reach the end page.")
                break
        ...
        try:
            for media_id in media_crawler_id_list:
                ...
                if TEST: self.last_news_url = None
                    ...
                    self.last_news_url = sorted(json.load(open(last_file)), key=lambda k: k['time'])[-1]['url']
                ...
                try:
                    ...
                    elif media == "聯合報(UDN News)-2020選舉": self.udn_all_reader_1()
                    elif media == "聯合報(UDN News)-要聞": self.udn_all_reader_2()
...
if __name__ == '__main__':
    ...
    #POLITICS = bool(int(raw_input('Sports: 0\tPolitics: 1\n'))) # forum is politics or sports
    #print('\nPlease input the media want to crawler')
    if POLITICS:
        media_list = ["自由時報(Liberty News)","蘋果日報(Apple Daily)","聯合報(UDN News)-2020選舉", "聯合報(UDN News)-要聞","中國時報(China Times)", # paper media
        ...
        "全部媒體(ALL)"]
    else: # sports media list
        media_list = ["自由時報(Liberty News)", "蘋果日報(Apple Daily)","聯合報(UDN News)","中國時報(China Times)", # paper media
        "TVBS", "三立新聞(SETN)", "非凡新聞(USTV)", "中央通訊社(CNA)",
        "關鍵評論網(The News Lens)", "大紀元(Epoch Times)", "新頭殼(Newtalk)",  
        "風傳媒(Storm Media)", "今日新聞(NOW News)", "世界日報(World Journal)", "民眾日報(Mypeople News)", "芋傳媒(Taro News)", 
        "Yahoo!奇摩新聞(YAHOO News)",
        "全部媒體(ALL)"]
    ...
    if TEST: crawler.read_news(str(2))
